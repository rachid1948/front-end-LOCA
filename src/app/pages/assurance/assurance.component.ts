...
export class AssuranceComponent implements AfterViewInit {
  ...
  records: AssuranceRecord[] = [];
  filteredRecords: AssuranceRecord[] = [];
  selectedYear: number = new Date().getFullYear();
  totalMontant: number = 0;
  vehicles: any[] = [];

  // Modal
  editRecord: AssuranceRecord | null = null;
  isNewRecord = false;

  constructor(private assuranceService: AssuranceService, private vehiculeService: VehiculeService) {}
    ...
    this.assuranceService.getAll().subscribe({
      next: (data: any[]) => {
        this.records = data.map(r => ({
          ...
          dateDebut: this.isoToFr(r.dateDebut ?? ''),
  ...
  private renderTotal(): void {
    const el = document.getElementById('assurance-total');
    if (el) el.textContent = this.totalMontant.toLocaleString('fr-MA') + ' DH';
    const cnt = document.getElementById('assurance-count');
    if (cnt) cnt.textContent = this.filteredRecords.length + ' véhicule(s)';
  }

  private renderTable(): void {
    const tbody = document.getElementById('assurance-tbody');
    if (!tbody) return;
    if (this.filteredRecords.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7" class="empty-row">Aucune assurance pour ${this.selectedYear}</td></tr>`;
      return;
    }
    tbody.innerHTML = this.filteredRecords.map((r, i) => {
      ...
      let badge = '';
      if (diffDays < 0) badge = '<span class="badge badge-exp">Expiré</span>';
      ...
      return `<tr>
        ...
        <td class="actions-cell">
          <button class="btn-edit" onclick="assuranceEdit(${r.id})">
          ...
          <button class="btn-del-row" onclick="assuranceDelete(${r.id})">
  ...
  private bindGlobals(): void {
    (window as any)['assuranceSetYear'] = (y: number) => {
    ...
    (window as any)['assuranceOpenNew'] = () => {
      this.editRecord = { id: 0, marque: '', modele: '', matricule: '', annee: this.selectedYear, dateDebut: '', dateExpiration: '', montant: 0 };
      this.isNewRecord = true;
      this.openModal();
    };

    (window as any)['assuranceEdit'] = (id: number) => {
    ...
    (window as any)['assuranceCloseModal'] = () => this.closeModal();

    (window as any)['assuranceSave'] = () => {
      ...
      const body = {
        ...
        dateDebut: f('modal-date-debut'),
        dateExpiration: f('modal-date-exp'),
        montant: Number(f('modal-montant')) || 0
      };
      if (!body.dateDebut || !body.dateExpiration) {
        this.showToast('Remplissez tous les champs obligatoires.', 'error');
        return;
      }
      const obs = this.isNewRecord
        ? this.assuranceService.create(body)
        : this.assuranceService.update(this.editRecord!.id, body);
    ...
    (window as any)['assuranceDelete'] = (id: number) => {
      if (!confirm('Supprimer cette assurance ?')) return;
      this.assuranceService.delete(id).subscribe({
        next: () => { this.showToast('Assurance supprimée.', 'success'); this.loadAll(); },
  ...
  private openModal(): void {
    const modal = document.getElementById('assurance-modal');
    ...
    setValue('modal-date-debut', this.isNewRecord ? '' : this.frToIso(this.editRecord.dateDebut));
    ...
    if (title) title.textContent = this.isNewRecord ? 'Nouvelle assurance' : 'Modifier l\'assurance';
    const delBtn = document.getElementById('modal-delete-btn');
    if (delBtn) delBtn.style.display = this.isNewRecord ? 'none' : 'inline-flex';
    if (!this.isNewRecord && delBtn) {
      (delBtn as HTMLButtonElement).onclick = () => (window as any)['assuranceDelete'](this.editRecord!.id);
    }
  }
